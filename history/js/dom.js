/*
 * dom Script for demonstration history API JavaScript Library
 *
...
/*
 * QSA CSS Selector Engine v1.2.3
 *
...
(function( window, undefined ){
	...
	var 
		// now ready
		ready = false,
		// stack for bind ready listener
		readyList = [],
		// object type list
		typeList = {},
		// mirror dom object
		mirror,
		// Поиск быстрых селекторов
		fastSelector = /^((?:\w+|[\*])$)|^(?:(\w+)|[\*])?\.([\w\-_]+$)|^(?:(\w+)|[\*])?#([\w\-_]+$)/,
	...
	dom.fn = dom.prototype = {
		...
		init: function( selector, context ) {
			...
			// Если selector не задан, прототип библиотеки.
			if ( !selector ) return this;

			// Если селектор строковой
			if ( typeof selector === "string") {

				if ( match = fastSelector.exec( selector ) ) {

					// Поиск по id
					if ( match[ 5 ] ) {

						if ( ( elem = document.getElementById( match[ 5 ] ) ) && elem.parentNode ) {
							if ( elem.id === match[ 5 ] ) {
								this[ 0 ] = elem;
								this.length = 1;
							} else {
								return dom.find( selector, context, this );
							}
						}
					...
					// ВЫЗОВ: ('body')
					// возвращаем тело документа
					if ( match[ 1 ] && match[ 1 ] == 'body' ) {
			...
			// если селектор является узлом HTML или XML
			...
			// Если селектор являеться функцией, выполним ее после загрузки DOM
			if ( dom.type( selector ) == 'function' ) {
				return mirror.ready( selector );
			}
	...
	dom.type = function( obj ) {
		return obj == null ? String( obj ) : typeList[ Object.prototype.toString.call( obj ) ] || "object";
	}

	dom.ready = dom.fn.ready = function( callback ) {
		if ( ready ) {
	...
	dom.find = function( selector, context, extra ) {

		extra = extra || dom.fn.constructor();

		return qsa.querySelectorAll( selector, context, extra );
	}

	dom.fn.find = function( selector, context ) {
		return dom.find( selector, this );
	}

	dom.fn.hide = function( ) {
		var i = 0, display, elem;

		for( var i = 0, j = this.length; i < j; i++ ) {

			elem = this[ i ];

			display = dom.style( elem )['display'];

			if ( display != 'none' ) {
				elem._oldStyleDisplay = display;
			}
	...
	dom.fn.show = function( ) {
		...
		for( ; i < j; i++ ) {
			...
			if ( elem._oldStyleDisplay ) {

				elem.style.display = elem._oldStyleDisplay;

			} else if ( !defaultDisplayView[ elem.nodeName ] ) {

				temp = document.createElement( elem.nodeName );
				document.body.appendChild( temp );
				defaultDisplayView[ elem.nodeName ] = elem.style.display = dom.style( temp )['display'];
				document.body.removeChild( temp );

			} else {

				elem.style.display = defaultDisplayView[ elem.nodeName ];
			}
		}
		return this;
	}
	...
	dom.fn.css = function( name, value ) {
		...
		var i, len = this.length, obj = {};

		if ( value === undefined && typeof name == "string" ) {
			return len == 0 ? "" : dom.style( this[ 0 ] )[ name ];
		}
	...
	dom.fn.attr = function( name, value ) {

		var len = this.length;

		if ( value === undefined ) {
			return len == 0 ? null : name == "href" ? this[ 0 ].getAttribute( name, 2 ) : this[ 0 ].getAttribute( name );
		}
	...
	dom.each = dom.fn.each = function( object, callback ) {

		if ( dom.type( object ) == 'function' ) {
			callback = object;
			object = this;
		}
	...
	dom.bind = function( elem, event, listener, capture ) {
		...
		if ( listener ) {
			...
			if ( !elem._eventsCache ) {
				elem._eventsCache = {};
			}
			if ( !elem._eventsCache[ event ] ) {
				elem._eventsCache[ event ] = [];
			}

			index = elem._eventsCache[ event ].push({
				orgListener: listener,
				midListener: function( e ) {

					e = dom.correctEvent( e, elem );

					if ( listener.call( e.target || e.srcElement || elem, e ) === false ) {
						e.preventDefault();
						return false;
					}
				},
			...
			if ( elem.addEventListener ) {
				// for Other event
				elem.addEventListener( event, elem._eventsCache[ event ][ index ].midListener, capture );
			} else if ( elem.attachEvent ) {
				// for IE event
				elem.attachEvent( 'on' + event, elem._eventsCache[ event ][ index ].midListener );
			}
		}
	}

	dom.unbind = function( elem, event, listener, capture ) {

		if ( !elem ) return;

		var events = elem._eventsCache,
			remove = [], i, len, evt,
			defCapture = capture || false;

		function findEvents( event, listener, all ) {

			for( i = events[ event ].length - 1; i >= 0; i-- ) {
				evt = events[ event ][ i ];

				if ( listener ) {
					if ( evt.orgListener === listener && evt.capture === defCapture ) {
						remove.push({
							event: event,
							listener: evt.midListener,
							capture: evt.capture
						});

						evt.orgListener = null;
						evt.midListener = null;
				...
				} else if ( capture === undefined || evt.capture === defCapture ) {
					remove.push({
						event: event,
						listener: evt.midListener,
						capture: evt.capture
		...
		if ( events && event && events[ event ] ) {
			...
			findEvents( event, listener );

		} else if ( events && !event ) {

			for( event in events ) {

				findEvents( event, listener, true );
			}
		...
		for( i = 0, j = remove.length; i < j; i++ ) {

			if ( elem.removeEventListener ) {
				elem.removeEventListener( remove[ i ].event, remove[ i ].listener, remove[ i ].capture );
			} else if ( elem.detachEvent ) {
				elem.detachEvent( 'on' + remove[ i ].event, remove[ i ].listener );
			}

			if ( remove[ i ].listener ) {
				remove[ i ].listener = null;
				delete remove[ i ]["listener"];
			}
		}
	...
	dom.trigger = function( elem, event ) {

		var o;

		if ( document.createEvent ) {
			o = document.createEvent( 'Events' );
			o.initEvent( event, true, true );
			elem.dispatchEvent( o );
		} else if( document.createEventObject ) {
			o = document.createEventObject();
			elem.fireEvent( "on" + event, o );
		}
	}

	dom.each( "bind unbind trigger".split(" "), function( index, name ) {

		dom.fn[ name ] = function( events, listener, capture ) {
			...
			for( i = 0; i < j; ) {
				...
				for( k = 0; k < length; ) {
					...
					dom[ name ]( elem, event === "" ? false : event, listener, capture );
				}
			}
	...
	dom.correctEvent = function( e, elem ) {
		...
		if ( !e.target ) {
			try {
				e.target = e.srcElement || elem || window;
			} catch( e ){}
		}
	...
	dom.each( ("blur focus focusin focusout load resize scroll unload click dblclick " +
		...
		dom.fn[ name ] = function( fn ) {
			return arguments.length > 0 ? this.bind( name, fn ) : this.trigger( name );
		}
	});

	dom.each( "html append".split(" "), function( index, name ) {

		dom.fn[ name ] = function( html ) {

			var i, len = this.length, child, fragment, tempDiv, script = [];

			if ( html === undefined ) {
				return len == 0 ? "" : this[ 0 ].innerHTML;
			}
			...
			for( i = 0; i < len; i++ ) {
				...
				try {
					this[ i ].appendChild( fragment );
				} catch ( _ ) {
					try {
						this[ i ].innerHTML += html;
					} catch ( _ ) {
						try {
							this[ i ].innerText += html;
						} catch ( _ ) {
							this[ i ].text += html;
						}
					}
	...
	dom.browser = {
		...
		opera: !!window.opera && window.opera.version && window.opera.version(),
		chrome: !!window.chrome && ( /chrome\/([\d\.]+)/i.exec( navigator.userAgent )[ 1 ] || true ),
		firefox: !!window.sidebar && ( /firefox\/([\d\.]+)/i.exec( navigator.userAgent )[ 1 ] || true ),
		safari: !window.external && !window.opera && ( /safari\/([\d\.]+)/i.exec( navigator.userAgent )[ 1 ] || true )
	}

	dom.browser.version = dom.browser.msie || dom.browser.opera || dom.browser.chrome || dom.browser.firefox || dom.browser.safari;


	var type = "Boolean Number String Function Array Date RegExp Object".split(' ');
	for( var k in type ) {
		typeList[ '[object ' + type[ k ] + ']' ] = type[ k ].toLowerCase();
	}
