<?php
	...
	/*
	*  Функция для получения информации о выбраном языке и прочих параметров из URL
	*  требует параметры двух таблиц "settings" и "lang"
	*  входные данные должны иметь формат получаемый функцией parse_uri, см. functions.php
	*/
	...
	* меня к написанию собственного парсера если встроенный не справится
	*/
	...
	/*
	*  Рабивает url на части, тербуеться для работы в этом сайте/движке
	*/
	function parse_uri( $url = null ) {

		$uri = _parse_url( $url == null ? preg_replace( "#^"._LINK_PATH."#iu", "/", $_SERVER['REQUEST_URI'] ) : $url );
		...
		if ( count( $out ) == 0 ) {
			...
			$result = array( 'module' => $out[ 2 ], 'params' => $out[ 4 ], 'uri' => $out[ 1 ].$out[ 3 ].$query );
		}
	...
	*/
	...
	/*
	*  Замена стандартной функции json_encode в PHP 5.2.x
	*  в более ранних версиях может отсутствовать, или выключена
	*  Работает без дополнительных параметров, простой вариант
	*/
	...
	/*
	*  Замена стандартной функции json_decode в PHP 5.2.x
	*  в более ранних версиях может отсутствовать, или выключена
	*  Работает без дополнительных параметров, простой вариант
	*/
